...
namespace impl {
	...
	MemoryMappedFileRw::MemoryMappedFileRw(MemoryMappedFileRw && other) : fd_{other.fd_}, data_{other.data_}, length_{other.length_} {
		other.fd_   = -1;
		other.data_ = nullptr;
	}

	MemoryMappedFileRw & MemoryMappedFileRw::operator=(MemoryMappedFileRw && other) {
		if (fd_ > -1) ::close(fd_);
	...
	estd::result<MemoryMappedFileRw, estd::error> MemoryMappedFileRw::create(std::string const & path, std::size_t size) {
		int fd = ::openat(AT_FDCWD, path.c_str(), O_RDWR | O_CLOEXEC | O_CREAT | O_TRUNC, 0644);
	...
	estd::result<MemoryMappedFileRw, estd::error> MemoryMappedFileRw::from_fd(int fd, std::size_t size) {
		...
		if (map == MAP_FAILED) return lastError("failed to map FD " + std::to_string(fd));
